...
Class Master extends DBConnection {
	...
	// 		exit;
	// 	}
	// 	//added this to validate email address
	...
	function register(){
		...
		$activation_code = bin2hex(openssl_random_pseudo_bytes(32));
		$activation_expiry = time() + (24 * 60 * 60); // 24 hours
		if (empty($id)) {
			$sql = "INSERT INTO `clients` set {$data}, `activation_code` = '{$activation_code}', `activation_expiry` = '{$activation_expiry}' ";
			$save = $this->conn->query($sql);
			$id = $this->conn->insert_id;
		} else {
			$sql = "UPDATE `clients` set {$data}, `activation_code` = '{$activation_code}', `activation_expiry` = '{$activation_expiry}' where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if ($save) {
			$resp['status'] = 'success';
	...
	function send_activation_email($email, $activation_code){
		...
		$message = "
			...
			Thank you for registering for our website. Please click on the following link to activate your account:
	
			https://www.yourwebsite.com/activate.php?email=$email&activation_code=$activation_code
